...
# The sitemap page contains multiple <ul> elements; the second one holds the links to flight deals
MIN_SITEMAP_ULS = 2
SITEMAP_LINKS_UL_INDEX = 1
...
def fetch_city_to_city_urls() -> list[str]:
    ...
    if len(all_uls) < MIN_SITEMAP_ULS:
        return []  # no flights found
    ul = all_uls[SITEMAP_LINKS_UL_INDEX]
...
def main() -> None:
    ...
    print(f"Creating database at {db_path}")  # noqa: T201
    ...
    try:
        ...
        print(f"Failed URLs: {len(all_error_urls)}")  # noqa: T201
